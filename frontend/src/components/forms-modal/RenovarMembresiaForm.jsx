...
export default function RenovarMembresiaForm({user, onClose}) {
  ...
  return (
     <form className="p-10 flex flex-col gap-2" onSubmit={handleSubmit}>
            <h1 className='text-center text-3xl font-bold border-b border-accent pb-2 mb-2'>Tipos de Membresia</h1>
            
                  <div className="flex flex-col ">
                    <ul className="flex flex-col gap-5 mt-2">
                      {data.map(membresia=><li className={`text-xl border border-border text-center rounded-md py-2 hover:bg-accent transition-all duration-300 cursor-pointer ${option === membresia._id ? "bg-accent" : null}`} onClick={()=>{setOption(membresia._id)}} key={membresia._id}>{capitalize(membresia.nombre)} - ${membresia.precio}</li>)}                      
                    </ul>
                  </div>
                  <button
                    type="submit"                    
                  ...
                  >{isPending ? <Loading/> : "Asignar Membresia"}</button>                    
                </form>
  )
}
